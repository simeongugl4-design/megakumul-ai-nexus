...
import {
  Search, ArrowRight, Globe, ExternalLink, BookOpen,
    ...
    >
      <div className="flex items-start justify-between gap-2 mb-2">
        <div className="flex items-center gap-2 text-xs text-muted-foreground">
          <Globe className="h-3 w-3" /><span className="truncate">{source.domain}</span>
        </div>
        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-muted text-[10px] font-bold text-muted-foreground shrink-0">{source.id}</span>
      </div>
      <h4 className="text-sm font-heading font-semibold text-foreground mb-1 line-clamp-2 group-hover:text-primary transition-colors">{source.title}</h4>
      ...
      <div className="mt-2 flex items-center gap-1 text-[10px] text-primary opacity-0 group-hover:opacity-100 transition-opacity">
        <span>Open source</span><ExternalLink className="h-2.5 w-2.5" />
      </div>
    </motion.a>
  );
}
...
export default function ResearchPage() {
  ...
  const topicWords = query.split(' ').slice(0, 4).join(' ');
  const followUpOptions = [
    { label: `Latest developments in ${topicWords}`, query: `What are the latest developments in ${topicWords}?` },
    { label: `Compare different perspectives on ${topicWords}`, query: `Compare different perspectives on ${topicWords}` },
    { label: `Practical applications of ${topicWords}`, query: `What are the practical applications of ${topicWords}?` },
    { label: `History and evolution of ${topicWords}`, query: `Explain the history and evolution of ${topicWords}` },
    { label: `Future predictions for ${topicWords}`, query: `What are future predictions and trends for ${topicWords}?` },
    { label: `Key statistics and data on ${topicWords}`, query: `What are the key statistics and data on ${topicWords}?` },
  ];
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {!hasResults ? (
          <div className="flex flex-1 flex-col items-center justify-center px-4 py-16 min-h-[calc(100vh-3.5rem)]">
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} className="mb-8 flex h-20 w-20 items-center justify-center rounded-2xl bg-secondary glow-secondary">
          ...
          <div className="mx-auto max-w-5xl px-4 py-8">
              ...
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="mb-6">
                <div className="flex items-center justify-between mb-3">
                  <h3 className="text-sm font-heading font-semibold text-foreground flex items-center gap-2">
                    <Globe className="h-4 w-4 text-secondary" /> Sources
            ...
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="rounded-2xl border border-border bg-card p-6 mb-6">
              {error ? (
                <div className="text-center py-8"><p className="text-destructive mb-2">⚠️ {error}</p><button onClick={handleNewResearch} className="text-sm text-primary hover:underline">Try again</button></div>
              ) : content ? (
                ...
                <div className="flex flex-col items-center py-12 gap-4">
                  <div className="flex items-center gap-2"><Loader2 className="h-5 w-5 animate-spin text-secondary" /><span className="text-sm text-muted-foreground">Analyzing and researching...</span></div>
                  <div className="flex gap-1.5"><span className="h-2 w-2 rounded-full bg-secondary typing-dot" /><span className="h-2 w-2 rounded-full bg-secondary typing-dot" /><span className="h-2 w-2 rounded-full bg-secondary typing-dot" /></div>
                </div>
              ) : null}
            </motion.div>
            ...
            {isComplete && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="mb-6">
                <h3 className="text-sm font-heading font-semibold text-foreground mb-3 flex items-center gap-2"><span className="text-secondary">📊</span> AI Diagrams</h3>
            ...
            {isComplete && (
              <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="mb-6">
                <FollowUpOptions
                  ...
                  onSelect={(q) => { setInput(q); research(q); }}
                  icon={Search}
                />
              </motion.div>
            )}
